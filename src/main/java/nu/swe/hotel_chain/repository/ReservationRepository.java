...
public interface ReservationRepository extends JpaRepository<Reservation, Integer> {
    ...
    @Query(value = "select case when count (R) > 0 then true else false end from Reservation as R where (not R.r_number = ?1) or (R.r_number = ?1 and R.hotel_id = ?2 and (?3 not between R.check_in and R.check_out))")
